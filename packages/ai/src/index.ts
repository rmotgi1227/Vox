...
export async function decideTurn(input: {
  ...
  const systemPrompt = [
    ...
    "ALREADY-GENERATED RULE: once you have generated an image this conversation, it STAYS on screen. If the shopper then asks about it ('do you have that picture?', 'where is it?', 'can I see it?', 'is it ready?'), do NOT generate again and do NOT change the screen — the image is already up. Reply acknowledging it ('Yep — it's right there on the screen.') with actions: []. NEVER emit a second generate for the same request.",
    "GENERATE BASE: before you generate an edit of a specific area (the trunk packed, the seats recolored), that area's REAL photo must be the one on screen so the edit starts from the right base. If the relevant photo isn't up yet, FIRST showImage it (and reply offering the generation) — only generate on the NEXT turn once the correct photo is on screen.",
...
function buildSpokenSystem(car: Car, alternatives?: CatalogMatch[]): string {
  ...
  return [
    ...
    "VISUALIZATION (Nano Banana): the screen can GENERATE a photoreal what-if image our stock photos can't show — the trunk PACKED for a trip, the seats in a different color, the car in a setting. When the shopper is unconvinced ('looks small', 'idk', 'hard to tell') OR asks to see a hypothetical/custom version ('show me it packed', 'what would it look like', 'can I get it in red'), OFFER to generate it ('want me to show you what it'd look like packed?') and STOP — do NOT just describe it in words instead. When they CONFIRM (yes / sure / go for it), reply with ONLY a brief 'On it — generating that now, give me a sec.' Do not narrate the imagined result; the screen will render it.",
...
export async function decideCanvas(input: {
  ...
  const system = [
    ...
    '  generate:   {"op":"generate","prompt":"<vivid photo-edit instruction>","baseRef":{"index":0}}   (AI-edits the REAL photo on screen into a believable "what-if" — e.g. trunk packed with luggage, seats in a different color. baseRef {"index":0} = the photo currently on screen.)',
    ...
    "- GENERATE (Nano Banana visualization): emit a generate action ONLY when recentTurns shows Vox just OFFERED to show a custom/what-if view (e.g. 'want me to show it packed?', 'want to see that in red?') AND the shopper's latest message CONFIRMS it (yes / sure / do it / show me / let's see it / go for it). When that happens, emit ONE action: {\"op\":\"generate\",\"prompt\":\"<vivid edit instruction>\",\"baseRef\":{\"index\":0}}. The prompt must describe the edit to the photo ON SCREEN concretely and photorealistically — e.g. 'Pack this open trunk full of realistic luggage: two large suitcases, a duffel bag, and grocery bags, fitting snugly, same lighting and camera angle' or 'Recolor the seats to a rich cognac brown leather, keep everything else identical'. Keep the same car, angle, and lighting. Do NOT generate on the FIRST mention of a concern — only after an offer was made and confirmed. Do NOT pair generate with other actions.",
